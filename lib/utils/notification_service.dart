...
class NotificationService {
  ...
  List<String> getExpiringPlans(DateTime targetDate) {
    log("sdgahdg");
    List<String> expiringPlans = [];

    for (var plan in mySeasonPassList) {
      log("AA:$plan");
      if (plan.endDate!.toDate().isBefore(targetDate)) {
        expiringPlans.add(plan.seasonPassModel!.passName.toString());
      }
    }
  ...
  //     ShowToastDialog.showToast("aaaaaaa");
  //     DateTime now = DateTime.now();
  //     DateTime oneWeekFromNow = now.add(const Duration(days: 7));
  //     log("duration:$oneWeekFromNow");
  //
  //     List<String> expiringPlans = await getExpiringPlans(oneWeekFromNow);
  //     log("BB:$expiringPlans");
  //     for (String plan in expiringPlans) {
  //       await showNotification(plan);
  //     }
  //   });
  // }

  Future<void> showNotification(String planName) async {
    AndroidNotificationChannel channel = const AndroidNotificationChannel(
      '0',
      'goRide-customer',
      description: 'Show QuickLAI Notification',
    ...
    await flutterLocalNotificationsPlugin.show(
      ...
      'Your $planName plan will expire in 1 week.',
