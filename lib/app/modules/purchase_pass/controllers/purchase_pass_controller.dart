...
class PurchasePassController extends GetxController {
  ...
  Rx<TextEditingController> identificationNoController = TextEditingController().obs;
  ...
  Rx<TextEditingController> companyRegistrationNoController = TextEditingController().obs;
  Rx<TextEditingController> addressController = TextEditingController().obs;
  RxString countryCode = "+60".obs;
  List<String> type = ["Pas Mingguan 1","Pas Mingguan 2","Pas Mingguan 3", "Pas Bulanan 1", "Pas Bulanan 6", "Pas Bulanan 12"];
  ...
  getArgument() async {
    ...
    if (argumentData != null) {
      ...
      selectedSessionPass.value = seasonPassList.where((p0) => p0.id == temp.id).first;
    }
    update();
  }
  ...
  addSeasonPassData() async {
    ...
    purchasePassModel.value.identificationNo = identificationNoController.value.text;
    ...
    purchasePassModel.value.companyRegistrationNo = companyRegistrationNoController.value.text;
    ...
    purchasePassModel.value.endDate =
        Timestamp.fromDate(DateTime.timestamp().add(Duration(days: checkDuration(selectedSessionPass.value.validity.toString()))));
  }
